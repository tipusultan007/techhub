                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="meta_keywords" id="meta_keywords" rows="2"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                            placeholder="e.g. keyword1, keyword2...">{{ old('meta_keywords', $page->meta_keywords) }}</textarea>
                    </div>
                    <div class="space-y-2">
            ...
            <div class="space-y-6">
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="redirect_url" id="redirect_url" value="{{ old('redirect_url', $page->redirect_url) }}"
                    ...
                    <div class="space-y-4 pt-4 border-t border-gray-100">
                        <div class="space-y-2">
                            ...
                            <select name="type" id="type" onchange="toggleReferenceField()"
                                ...
                                <option value="static" {{ old('type', $page->type) == 'static' ? 'selected' : '' }}>Static Page (Standard)</option>
                                <option value="category" {{ old('type', $page->type) == 'category' ? 'selected' : '' }}>Mirror Category Page</option>
                                <option value="product" {{ old('type', $page->type) == 'product' ? 'selected' : '' }}>Mirror Product Page</option>
                            </select>
                            <p class="text-xs text-gray-500">Choose "Mirror" to show content from another page.</p>
                        </div>

                        <!-- Category Selection -->
                        <div id="category_field" class="space-y-2 {{ old('type', $page->type) == 'category' ? '' : 'hidden' }}">
                            ...
                            <select name="reference_id_category" id="category_id" 
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('reference_id', $page->reference_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <!-- Product Selection -->
                        <div id="product_field" class="space-y-2 {{ old('type', $page->type) == 'product' ? '' : 'hidden' }}">
                            ...
                            <select name="reference_id_product" id="product_id" 
                                ...
                                @foreach($products as $prod)
                                    <option value="{{ $prod->id }}" {{ old('reference_id', $page->reference_id) == $prod->id ? 'selected' : '' }}>{{ $prod->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        <!-- Hidden fields for the actual reference_id -->
                        <input type="hidden" name="reference_id" id="reference_id" value="{{ old('reference_id', $page->reference_id) }}">
                    </div>

                    <div class="space-y-2 pt-4 border-t border-gray-100">
                        <label for="canonical_url" class="text-sm font-semibold text-gray-700">Canonical URL (Optional)</label>
                        <input type="text" name="canonical_url" id="canonical_url" value="{{ old('canonical_url', $page->canonical_url) }}"
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 transition-all outline-none"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="show_on_footer" id="show_on_footer" 
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 transition-all outline-none">
                            <option value="0" {{ old('show_on_footer', $page->show_on_footer) == '0' ? 'selected' : '' }}>No (Hide from Footer)</option>
                            <option value="1" {{ old('show_on_footer', $page->show_on_footer) == '1' ? 'selected' : '' }}>Yes (Show in Footer)</option>
                        </select>
                    </div>
