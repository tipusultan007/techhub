...
@section('header', 'Edit AMC Template')
...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-2xl font-bold text-gray-800">Edit Template: {{ $amcTemplate->name }}</h2>
    </div>

    <form action="{{ route('amc-templates.update', $amcTemplate) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ $amcTemplate->name }}" required class="w-full px-4 py-3 rounded-xl border-gray-100 focus:border-emerald-500 focus:ring-emerald-500/20 transition-all" placeholder="e.g., Annual Maintenance Agreement">
                </div>

                <div class="space-y-2">
                    <label class="text-xs font-black text-gray-400 uppercase tracking-widest">Agreement Content</label>
                    <textarea name="content" id="template-editor" class="tinymce">{{ $amcTemplate->content }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_default" value="1" {{ $amcTemplate->is_default ? 'checked' : '' }} class="sr-only peer">
