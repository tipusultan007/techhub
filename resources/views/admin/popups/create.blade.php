...
@section('header', 'Create Offer Popup')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6 flex items-center justify-between">
        <div class="flex-1 min-w-0">
            <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">Create New Popup</h2>
            <nav class="flex text-sm font-medium text-gray-500 mt-2" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2">
                    ...
                    <li>
                        <span class="text-gray-900" aria-current="page">Create</span>
                    </li>
                </ol>
            </nav>
        </div>
    ...
    <form action="{{ route('popups.admin.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" required
                                ...
                                placeholder="e.g. Flash Sale 50% Off" value="{{ old('title') }}">
                        ...
                        <div>
                            ...
                            <textarea name="subtitle" id="subtitle" rows="3"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border"
                                placeholder="Brief description">{{ old('subtitle') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="button_text" id="button_text"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border"
                                    placeholder="e.g. Shop Now" value="{{ old('button_text', 'Shop Now') }}">
                            </div>
                            <div>
                                <label for="link" class="block text-sm font-medium text-gray-700">Target Link</label>
                                <input type="text" name="link" id="link"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border"
                                    placeholder="e.g. /shop" value="{{ old('link', '#') }}">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="display_delay" id="display_delay" min="0"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border"
                                value="{{ old('display_delay', 5) }}">
                            <p class="mt-1 text-xs text-gray-500">Wait time before showing popup.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="cookie_duration" id="cookie_duration" min="1"
                                class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2.5 border"
                                value="{{ old('cookie_duration', 1) }}">
                            <p class="mt-1 text-xs text-gray-500">Days to hide after closing.</p>
                        </div>
