...
@section('header', 'Edit AMC Contract')
...
<div class="space-y-6">
    ...
    <form action="{{ route('amcs.update', $amc) }}" method="POST" id="amc-form" class="space-y-6" enctype="multipart/form-data">
        ...
        <div id="full-width-editor" class="{{ $amc->custom_agreement_content ? '' : 'hidden' }} space-y-4">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                <textarea name="custom_agreement_content" id="custom-agreement-editor" class="tinymce">{{ $amc->custom_agreement_content }}</textarea>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="customer_id" required class="w-full select2-basic">
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ $amc->customer_id == $customer->id ? 'selected' : '' }}>{{ $customer->name }} ({{ $customer->phone }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-gray-400 uppercase tracking-widest">Total Amount</label>
                            <input type="number" step="0.01" name="amount" value="{{ $amc->amount }}" required class="w-full form-input-styled form-input-styled px-4 py-3 rounded-xl border-gray-200 focus:border-emerald-500 focus:ring-emerald-500/20 transition-all" placeholder="0.00">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="start_date" value="{{ $amc->start_date->format('Y-m-d') }}" required class="w-full form-input-styled px-4 py-3 rounded-xl border-gray-200 focus:border-emerald-500 focus:ring-emerald-500/20 transition-all">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-gray-400 uppercase tracking-widest">End Date</label>
                            <input type="date" name="end_date" value="{{ $amc->end_date->format('Y-m-d') }}" required class="w-full form-input-styled px-4 py-3 rounded-xl border-gray-200 focus:border-emerald-500 focus:ring-emerald-500/20 transition-all">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div class="space-y-2">
                            ...
                            <select name="frequency" required class="w-full select2-basic">
                                <option value="monthly" {{ $amc->frequency == 'monthly' ? 'selected' : '' }}>Monthly</option>
                                <option value="quarterly" {{ $amc->frequency == 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                                <option value="semi-annually" {{ $amc->frequency == 'semi-annually' ? 'selected' : '' }}>Semi-Annually</option>
                                <option value="annually" {{ $amc->frequency == 'annually' ? 'selected' : '' }}>Annually</option>
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-black text-gray-400 uppercase tracking-widest">Status</label>
                            <select name="status" required class="w-full select2-basic">
                                <option value="pending" {{ $amc->status == 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="active" {{ $amc->status == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="expired" {{ $amc->status == 'expired' ? 'selected' : '' }}>Expired</option>
                                <option value="cancelled" {{ $amc->status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div id="items-container" class="space-y-3">
                        @foreach($amc->items as $index => $amcItem)
                        <div class="item-row grid grid-cols-1 md:grid-cols-12 gap-4 p-4 bg-gray-50 rounded-xl relative group">
                            <div class="md:col-span-5 space-y-1">
                                ...
                                <select name="items[{{ $index }}][product_id]" class="w-full select2-basic">
                                    <option value="">-- Choose Product --</option>
                                    @foreach($products as $product)
                                        <option value="{{ $product->id }}" {{ $amcItem->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="md:col-span-6 space-y-1">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Description/Serial No.</label>
                                <input type="text" name="items[{{ $index }}][description]" value="{{ $amcItem->description }}" required class="w-full form-input-styled px-3 py-2 rounded-lg border-gray-200 text-sm" placeholder="e.g., HP LaserJet M1132 - S/N: 12345">
                            </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div id="services-container" class="space-y-3">
                        @forelse($amc->includedServices as $index => $service)
                        <div class="service-row grid grid-cols-1 md:grid-cols-12 gap-4 p-4 bg-gray-50 rounded-xl relative group">
                            <div class="md:col-span-4 space-y-1">
                                ...
                                <input type="text" name="included_services[{{ $index }}][service_name]" value="{{ $service->service_name }}" class="w-full form-input-styled px-3 py-2 rounded-lg border-gray-200 text-sm" placeholder="e.g., Cleaning">
                            </div>
                            <div class="md:col-span-7 space-y-1">
                                ...
                                <input type="text" name="included_services[{{ $index }}][description]" value="{{ $service->description }}" class="w-full form-input-styled px-3 py-2 rounded-lg border-gray-200 text-sm" placeholder="e.g., Full internal & external cabinet cleaning">
                            </div>
                            <div class="md:col-span-1 flex items-end justify-center pb-1">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
                    ...
                    <div class="space-y-4">
                        <div class="flex p-1 bg-gray-50 rounded-xl">
                            <button type="button" onclick="setAgreementType('template')" id="btn-type-template" class="flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all {{ $amc->custom_agreement_content ? 'text-gray-400' : 'bg-white shadow-sm text-emerald-600' }}">
                                Use Template
                            </button>
                            <button type="button" onclick="setAgreementType('custom')" id="btn-type-custom" class="flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-lg transition-all {{ $amc->custom_agreement_content ? 'bg-white shadow-sm text-emerald-600' : 'text-gray-400' }}">
                                Custom Write
                            </button>
                            <input type="hidden" name="agreement_type" id="agreement_type" value="{{ $amc->custom_agreement_content ? 'custom' : 'template' }}">
                        </div>

                        <div id="wrapper-template" class="space-y-2 {{ $amc->custom_agreement_content ? 'hidden' : '' }}">
                        ...
                        <div id="wrapper-custom-summary" class="{{ $amc->custom_agreement_content ? '' : 'hidden' }} p-4 bg-emerald-50 rounded-xl border border-emerald-100 text-center">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="notes" rows="4" class="w-full form-input-styled px-4 py-3 rounded-xl border-gray-200 focus:border-emerald-500 focus:ring-emerald-500/20 transition-all" placeholder="Any private details...">{{ $amc->notes }}</textarea>
                    </div>
...
<script>
    ...
    let serviceIndex = {{ $amc->includedServices->count() ?: 1 }};
