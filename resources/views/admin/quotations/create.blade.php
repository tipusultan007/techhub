...
<body class="bg-gray-100 h-screen w-screen overflow-hidden flex flex-col font-sans">
    ...
    <div class="flex-1 flex overflow-hidden">
                    ...
                    @foreach ($initialProducts as $p)
                        ...
                        <div class="product-card bg-white rounded-lg shadow-sm border border-gray-200 cursor-pointer hover:shadow-md hover:border-blue-400 transition relative overflow-hidden group"
                            ...
                            <div class="p-3">
                                ...
                                @else
                                    <div class="mt-2 text-xs text-gray-400 font-mono">{{ $p['sku'] }}</div>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="w-full md:w-1/2 bg-white flex flex-col shadow-2xl z-10">
            ...
            <div class="p-4 border-t bg-gray-50">
                ...
                <textarea id="quotation_notes" rows="3" class="w-full border border-gray-300 rounded p-2 text-sm focus:border-blue-500 outline-none" placeholder="Enter terms or notes here...">{{ settings('quotation_notes') }}</textarea>
            </div>
            ...
            <div class="bg-slate-50 p-4 border-t border-slate-200">
                ...
                <button class="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 shadow-lg transform active:scale-95 transition flex items-center justify-center gap-2" onclick="saveQuotation()">
    ...
    <div id="processingOverlay" class="fixed inset-0 bg-white bg-opacity-80 hidden items-center justify-center z-50">
        <div class="text-center">
            ...
            <h2 class="text-xl font-bold text-gray-700">Saving Quotation...</h2>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            @isset($duplicateData)
                ...
                $('#quotation_notes').val(`{!! $duplicateData['quotation_notes'] !!}`);
                renderCart();
            @endisset
        });
                ...
                $.get('{{ route("quotations.search") }}', { term: term }, function(data) {
                    ...
                    if (data.length === 0) {
                        ...
                        data.forEach(p => {
                            ...
                            if (p.type !== 'service') {
                                stockHtml = p.stock > 0 ? 
                                    `<div class="text-[10px] text-green-600 font-bold">${p.stock} In Stock</div>` : 
                                    `<div class="text-[10px] text-red-600 font-bold">Out of Stock</div>`;
                            }
                            html += `
        ...
        function renderCart() {
            ...
            if (cart.length === 0) {
                ...
                cart.forEach((item, index) => {
                    ...
                    html += `
                    <tr class="border-b">
                        <td class="p-2 text-xs font-medium w-[25%]">
                            ${item.type === 'service' ? 
        ...
        function saveQuotation() {
            ...
            $.ajax({
                ...
                error: function(err) {
                    ...
                    alert('Error: ' + (err.responseJSON ? err.responseJSON.message : 'Failed to save'));
                }
            });
        }
    </script>
